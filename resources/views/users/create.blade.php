...
@section('content')
    <div class="mx-auto max-w-2xl space-y-4">
        <div>
            <h1 class="text-xl font-extrabold text-slate-900">Tambah User</h1>
            <p class="text-sm text-slate-500">Buat akun untuk admin atau petugas.</p>
        </div>
        <form method="POST" action="{{ route('users.store') }}"
            class="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">@csrf
            @include('users.form')
